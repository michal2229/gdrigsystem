...
void RigSystemCommon::add_node(Node n) {
  ...
  DEBUG_PRINT("[RigSystemCommon::add_node] n.pos = ( " + String::num(n.pos.x) +
              ", " + String::num(n.pos.y) + ", " + String::num(n.pos.z) + " )");
}
...
void RigSystemCommon::add_conn(Conn c) {
  ...
  DEBUG_PRINT("[RigSystemCommon::add_conn] c.brk_thr = " +
              String::num(c.brk_thr));
  DEBUG_PRINT("[RigSystemCommon::add_conn] c.broken = " +
              String::num(c.broken));
}
...
void RigSystemCommon::integrate_system_radau2(float dt) {
  ...
  for (int i = 0; i < c::system::max_iter_num; ++i) {
    for (size_t j = 0; j < num_nodes; ++j) {
      ...
      m_s.pos1[j] = (npos + dt * (c::radau2::A11 * m_s.K1_vel[j] +
                                  c::radau2::A12 * m_s.K2_vel[j]));
      m_s.vel1[j] = (nvel + dt * (c::radau2::A11 * m_s.K1_acc[j] +
                                  c::radau2::A12 * m_s.K2_acc[j]));
      m_s.pos2[j] = (npos + dt * (c::radau2::A21 * m_s.K1_vel[j] +
                                  c::radau2::A22 * m_s.K2_vel[j]));
      m_s.vel2[j] = (nvel + dt * (c::radau2::A21 * m_s.K1_acc[j] +
                                  c::radau2::A22 * m_s.K2_acc[j]));
    }
  ...
  for (size_t i = 0; i < num_nodes; ++i) {
    ...
    if (!m_s.nodes_pinned[i]) {
      npos = (npos + dt * (c::radau2::b1 * m_s.K1_vel[i] +
                           c::radau2::b2 * m_s.K2_vel[i]));
      nvel = (nvel + dt * (c::radau2::b2 * m_s.K1_acc[i] +
                           c::radau2::b1 * m_s.K2_acc[i]));
    } else {
      // npos = (npos);
